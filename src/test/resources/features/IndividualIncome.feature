@local
Feature:  Local - As Individual-Income api consumer I should be able to view all income data

  Scenario Outline: validate success responses from individual-income api call
    Given I have generated a test user for individual-income with the UTR: <utr> for: <taxYear> with scenario: <scenario json>
    And I have generated a bearer token Valid for the UTR '<utr>'
    Then I should get the <statusCode> success status when calling individual-income with UTR: <utr> and tax year: <taxYear> for scenario: <scenario json>
  ...
  Scenario Outline: validate error responses from individual-income api call
    Given I have generated a test user for individual-income with the UTR: <utr> for: <taxYear> with scenario: <scenario json>
    And I have generated a bearer token <bearerToken> for the UTR '<utr>'
    Then I should get the statusCode: <statusCode> responseCode: <code> responseMessage: <message> when calling individual-income with UTR: <utr> and tax year: <taxYear>
